class Prueba
  ...
  def llamar
    ...
    self.deberia ser 7
    self.deberia ser_hola
    self.deberia tener_merlusa 1
    self.deberia tener_merlusa menor_a 2
    self.deberia tener_asd 1
    self.deberia entender :ser_
    {}.deberia explotar_con ZeroDivisionError
  end
  ...
  def entender(mensaje)
    return [Proc.new do self.methods.any?{|me| me.eql? mensaje} ||
        begin
          self.clone.method_missing(mensaje)
          true
...
##Metodo para chequear todos los metodos de instancia de todas las clases
ObjectSpace.each_object(Class).each do
|valor| puts valor.instance_methods
end
